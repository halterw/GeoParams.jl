# Script copied from: https://github.com/PTsolvers/Stokes2D_simpleVEP/blob/add-GeoParams-examples/GeoParams_PT_examples/scripts/Stokes2D_VEP_reg_ctau_GP_dimensional.jl
...
############### START ADDING
# Functions
# Cartesian to Rotated coordinate system
function CartToRot(Axx,Ayy,Axy,Ayx,Q11,Q22,Q12,Q21)
    Axx_rot =  Q11 .* (Axx .* Q11 + Ayx .* Q12) + Q12 .* (Axy .* Q11 + Ayy .* Q12)
    Ayy_rot =  Q21 .* (Axx .* Q21 + Ayx .* Q22) + Q22 .* (Axy .* Q21 + Ayy .* Q22)
    Axy_rot =  Q21 .* (Axx .* Q11 + Ayx .* Q12) + Q22 .* (Axy .* Q11 + Ayy .* Q12)
    Ayx_rot =  Q11 .* (Axx .* Q21 + Ayx .* Q22) + Q12 .* (Axy .* Q21 + Ayy .* Q22)
...
function RotToCart(Axx_rot,Ayy_rot,Axy_rot,Ayx_rot,Q11,Q22,Q12,Q21)
    ...
    Ayx =  Q11 .* (Axx_rot .* Q12 + Ayx_rot .* Q22) + Q21 .* (Axy_rot .* Q12 + Ayy_rot .* Q22)
    return Axx, Ayy, Axy, Ayx
end
############### END ADDING
...
@views function Stokes2D_vep(doPlot, nt)
    ...
    ϕ       = 30.0*do_DP
    μ0      = 1e20Pas            # viscous viscosity
    μi      = 1e19Pas            # viscous viscosity perturbation
    G0      = 1e12Pa             # elastic shear modulus
    ...
    MatParam = (SetMaterialParams(Name="Matrix"   , Phase=1,
                ...
                CompositeRheology = CompositeRheology(SetConstantElasticity(G=Gi, ν=0.5),LinearViscous(η=10*μ0), pl), CharDim=CharDim),
                )
    ...
    μi      = nondimensionalize(μi, CharDim)   
   ...
   # nx, ny  = 128, 128           # numerical grid resolution
    ...
    tol_VEit    = 1.0e-12        # VE trial stress: nonlinear tolerence
    nitmax_VEit = 20             # VE trial stress: max number of iters
    noisy_VEit  = 4              # VE trial stress: switch to print additional info
    # Preprocessing
    dx, dy  = Lx/nx, Ly/ny
    ...
    aniface = ones(Dat, nx  ,ny  ) * 2.0        # elastic anisotropy factor []
    anifacve = zeros(Dat, nx  ,ny  )            # init visco-elastic anisotropy factor []
    # Initial condition
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            for i = 1:size(Exx,1)
                for j = 1:size(Exx,2)
                    Txx_rot[i,j],Tyy_rot[i,j],Txy_rot[i,j], η_ve[i,j], anifacve[i,j] = LocalViscoElasticTrialStress_Newton(Exx_rot[i,j], Eyy_rot[i,j], Exy_rot[i,j], Txx_o_rot[i,j], Tyy_o_rot[i,j], Txy_o_rot[i,j], η_v[i,j], η_e[i,j], anifacv[i,j], aniface[i,j], tol_VEit, nitmax_VEit, noisy_VEit)
                end
            end

            η_vep = η_ve   # just for plotting...
            # rotate back
            Tyx_rot = Txy_rot
